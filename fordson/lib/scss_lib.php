...
function theme_fordson_get_pre_scss($theme) {
	...
	// Set the default image for the header. Mihir slide4
    $slide4image = $theme->setting_file_url('slide4image', 'slide4image');
    if (isset($slide4image)) {
        // Add a fade in transition to avoid the flicker on course headers ***.
        $prescss .= '.slide4image {background-image: url("'.$slide4image.'"); background-size:cover; background-repeat: no-repeat; background-position:center;}';
    }
